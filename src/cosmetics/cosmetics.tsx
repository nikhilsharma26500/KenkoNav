...
const formFields: FormField[] = [
  ...
  { label: "Restrictions", name: "restrictions", type: "text", placeholder: "Restrictions" },
...
const Cosmetics: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${PRODUCTION_URL}/cosmetics/set_model_response_cosmetics`, {
        method: 'POST',
        body: data,
        headers: {
    ...
    <section className="flex flex-col justify-center items-center h-screen text-center gap-y-10 mx-auto container px-5 sm:px-0">
      <div className="gap-y-10">
        <span className='flex flex-row justify-center items-start w-full gap-x-4'>
          ...
          <div className="rounded-full bg-indigo-500 px-2 text-lg handjet font-bold">Cosmetics</div>
        </span>
